    ...
    >
      <div className="flex items-center justify-center flex-1 gap-14">
        <div className="relative w-[380px] h-[380px] flex-shrink-0">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[110px] h-[110px] bg-[#015546] rounded-full flex items-center justify-center text-white font-extrabold text-sm text-center shadow-[0_0_0_6px_rgba(1,85,70,0.15),0_0_0_16px_rgba(1,85,70,0.06)]">
          ...
          {nodes.map((n) => (
            <div key={n.label} className={`absolute w-[120px] h-[100px] bg-white rounded-xl border-2 border-slate-200 flex flex-col items-center justify-center text-center p-2.5 shadow-sm ${n.pos}`}>
              ...
              <div className="text-[13px] font-bold text-slate-800">{n.label}</div>
              <div className="text-[11px] text-slate-500 mt-px">{n.sub}</div>
            </div>
          ))}
        </div>
        <div className="max-w-[360px]">
          {steps.map((step, i) => (
            <div key={i} className="flex gap-3 mb-4 items-start">
              <div className={`w-6 h-6 min-w-[24px] rounded-full text-white text-[11px] font-bold flex items-center justify-center ${step.special ? "bg-[#FF8F1C]" : "bg-[#015546]"}`}>
                {step.special ? "↻" : i + 1}
              </div>
