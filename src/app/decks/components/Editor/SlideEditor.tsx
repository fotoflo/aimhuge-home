...
export function SlideEditor({ initialSlides, deckSlug, viewerPath }: SlideEditorProps) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const iframeSrc = useMemo(() => viewerPath ? `${viewerPath}?edit=true#slide-${current + 1}` : `/decks/${deckSlug}?edit=true#slide-${current + 1}`, [refreshKey, deckSlug, viewerPath]);
