...
- Always use a centralized `getURL()` method to assemble the host origin (`NEXT_PUBLIC_SITE_URL` vs `NEXT_PUBLIC_VERCEL_URL`) rather than raw `window.location.origin`, and avoid passing query parameters directly to Supabase Auth's `redirectTo` unless wildcard records (`**`) are explicitly configured in the cloud dashboard.
